...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for center_word_idx, center_word in enumerate(sentence):
            word_window = []
            start_window_idx, end_window_idx = center_word_idx - window_size, center_word_idx + window_size

            for curr_word_idx in range(start_window_idx, end_window_idx + 1):
                if curr_word_idx < 0:
                    curr_word = start
                elif curr_word_idx >= len(sentence):
                    curr_word = end
                else:
                    curr_word = sentence[curr_word_idx]

                word_window += curr_word

            windowed_data.append((word_window, labels[center_word_idx]))
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable('b1', (self.config.hidden_size,), tf.float32, tf.zeros_initializer())

        U = tf.get_variable('U', (self.config.hidden_size, self.config.n_classes),
                            tf.float32, tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable('b2', (self.config.n_classes,), tf.float32, tf.zeros_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        """
        sample_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
